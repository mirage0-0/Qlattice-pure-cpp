...
if [ -z "$prefix" ] && [ -z "$INSTALL_PREFIX" ]; then
    echo "错误: 请设置 prefix 环境变量，例如: prefix=/path/to/install ./build-grid-neonv8.sh"
    echo "或者设置 INSTALL_PREFIX 环境变量"
    exit 1
fi


echo "=== 构建 Grid (Linux HPC NEONv8) ===" | tee -a "$LOG_FILE"
...
# NEONv8 配置: 使用 ARMv8 NEON (128-bit)
export CXXFLAGS="-fPIC -DUSE_QLATTILE -fopenmp -w -Wno-psabi -march=armv8-a"
export CFLAGS="-fPIC"
...
echo ">>> 配置 Grid (Linux HPC NEONv8)" | tee -a "$LOG_FILE"
mkdir -p build
cd build
