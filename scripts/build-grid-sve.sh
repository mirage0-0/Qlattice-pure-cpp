...
echo "=== 构建 Grid (Linux HPC SVE) ===" | tee -a "$LOG_FILE"
...
export CXXFLAGS="-fPIC -DUSE_QLATTILE -fopenmp -w -Wno-psabi -march=armv8-a+sve -msve-vector-bits=256"
export CFLAGS="-fPIC"
...
echo ">>> 配置 Grid (Linux HPC SVE 256-bit)" | tee -a "$LOG_FILE"
mkdir -p build
cd build
